...
for i in range(len(countries)):
    ...
    confirmed_pred = pred_logistic(days, confirmed_region, days_prediction, population[i]*herd_imm_ratio)
    deaths_pred = pred_logistic(days, deaths_region, days_prediction, population[i]*herd_imm_ratio)
    recovered_pred = pred_logistic(days, recovered_region, days_prediction, population[i]*herd_imm_ratio)
    ...
    loc2_str = "nan,"
    n_str = str(confirmed_pred[-1])+",nan,"
    r_str = str(deaths_pred[-1])+",nan,"
    d_str = str(recovered_pred[-1]) + ",nan,"
